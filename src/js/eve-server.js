...
const { findProfiles, readDefaultFolders } = require('./eve-folder');
...
async function changeServer(server) {
  ...
  const title = locale.servers[server] || server;
...
async function getServerStatus() {
  ...
  let status = 'N/A';
  let players = 'N/A';
  let cssClass = 'text-warning fw-bold';
  ...
  serverStatus.text(status).attr('class', cssClass);
  playerCount.text(players).attr('class', cssClass);
}
